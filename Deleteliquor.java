...
public class DeleteLiqour extends JFrame implements ActionListener
{
	JFrame jf;
...
l1.setBounds(50,100,200,25);
		...
		t1.setBounds(250,100,100,25);t1.setToolTipText("Enter Liqour batch no to delete ");
		jf.add(t1);
  ...
  l2.setBounds(50,140,200,25);
		...
		t2.setBounds(250,140,200,25);t2.setToolTipText("Enter liqour name to delete ");
		jf.add(t2);
  ...
  l9.setBounds(470,180,200,25);
   ...
   l11.setBounds(470,260,180,25);
       ...
       l12.setBounds(470,300,180,25);
		...
		b1.setBounds(300,330,110,35);b1.setToolTipText("click to delete liqour details");
    ...
    if(ae.getSource()==b0)
	  {
	  	try
	  	{
	  	 if(((t1.getText()).equals(""))&&((t2.getText()).equals("")))
	        {
		    JOptionPane.showMessageDialog(this,"Please enter liqour bno or name !","Warning!!!",JOptionPane.WARNING_MESSAGE);
	        }
	        else
	        {
	         int foundrec = 0;
		...
		System.out.println("Connected to database.");
		  ...
		  ps=con.prepareStatement("select * from Liqour where mname='"+ t2.getText()+"' or mbno='"+t1.getText()+"'");
		  rs=ps.executeQuery();
		  while(rs.next())
	      {
	 ...
	 {//delete
		...
		con=DriverManager.getConnection("jdbc:mysql://localhost:3306/medical_store","root","");
		System.out.println("Connected to database.");
	    ps=con.prepareStatement("delete from liqour where mbno='"+t1.getText()+"' or mname='"+t2.getText()+"'");
	    ps.executeUpdate();
